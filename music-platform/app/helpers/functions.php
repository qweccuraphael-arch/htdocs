...
<div class="wrap">
  ...
  <div class="foot">
&copy; %YEAR% BeatWave &mdash; Ghana's Music Hub<br>
...
function sendPayoutNotificationEmail(string $to, string $artistName, float $amount, string $status, string $paymentMethod, ?string $notes = null): bool {
    $methodLabel = ucfirst(str_replace('_', ' ', $paymentMethod));
    $statusLabel = ucfirst($status);
    $message = "Hi <strong>{$artistName}</strong>,<br><br>";

    if ($status === 'approved') {
        $message .= "Your payout request for <strong>" . formatMoney($amount) . "</strong> via <strong>{$methodLabel}</strong> has been approved and is being processed.";
    } elseif ($status === 'paid') {
        $message .= "Your payout of <strong>" . formatMoney($amount) . "</strong> via <strong>{$methodLabel}</strong> has been marked as paid.";
    } elseif ($status === 'rejected') {
        $message .= "Your payout request for <strong>" . formatMoney($amount) . "</strong> via <strong>{$methodLabel}</strong> was rejected.";
    } else {
        $message .= "Your payout request status is now <strong>{$statusLabel}</strong>.";
    }

    if ($notes) {
        $message .= "<br><br><strong>Admin note:</strong> " . htmlspecialchars($notes);
    }

    $message .= "<br><br><a href='" . APP_URL . "/artist/payments.php' class='btn'>View Payment Status</a>";

    return sendEmail($to, "Payout {$statusLabel} — " . formatMoney($amount), emailTemplate("Payout {$statusLabel}", $message));
}

function sendPayoutNotificationSMS(string $phone, string $artistName, float $amount, string $status, string $paymentMethod): array {
    $methodLabel = ucfirst(str_replace('_', ' ', $paymentMethod));
    $statusText = $status === 'paid' ? 'paid' : ($status === 'approved' ? 'approved' : ($status === 'rejected' ? 'rejected' : $status));
    $msg = "Hi {$artistName}! Your payout of " . formatMoney($amount) . " via {$methodLabel} has been {$statusText}. Check BeatWave payments for details.";
    return sendSMS($phone, $msg);
}
