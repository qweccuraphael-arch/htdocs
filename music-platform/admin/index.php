...
require_once dirname(__DIR__) . '/app/models/Stream.php';
...
$totalStreams = $streamModel->countTotal();
$artistEarnings = $earningsModel->platformTotal();
...
$todayStreams = $streamModel->countToday();
$pendingPayouts = $payoutModel->getPendingTotal();
...
<div class="panel-layout">
  ...
  <main class="panel-main">
    ...
    <div class="stats-grid">
      <div class="stat-card"><div class="stat-icon">S</div><div class="stat-value"><?= number_format($totalSongs) ?></div><div class="stat-label">Total Songs</div></div>
      <div class="stat-card"><div class="stat-icon">A</div><div class="stat-value"><?= number_format($totalArtists) ?></div><div class="stat-label">Artists</div></div>
      <div class="stat-card"><div class="stat-icon">D</div><div class="stat-value"><?= formatNumber($totalDownloads) ?></div><div class="stat-label">Total Downloads</div></div>
      <div class="stat-card"><div class="stat-icon">P</div><div class="stat-value"><?= formatNumber($totalStreams) ?></div><div class="stat-label">Total Streams</div></div>
      <div class="stat-card"><div class="stat-icon">M</div><div class="stat-value"><?= formatMoney($adminEarnings) ?></div><div class="stat-label">Admin Earnings</div></div>
      <div class="stat-card"><div class="stat-icon">R</div><div class="stat-value"><?= formatMoney($artistEarnings) ?></div><div class="stat-label">Artist Earnings</div></div>
      <div class="stat-card"><div class="stat-icon">Q</div><div class="stat-value" style="color:#ffc107"><?= number_format($pendingSongs) ?></div><div class="stat-label">Pending Approval</div></div>
      <div class="stat-card"><div class="stat-icon">W</div><div class="stat-value" style="color:#2196f3"><?= formatMoney($pendingPayouts) ?></div><div class="stat-label">Pending Payouts</div></div>
      <div class="stat-card"><div class="stat-icon">T</div><div class="stat-value"><?= number_format($todayDownloads + $todayStreams) ?></div><div class="stat-label">Plays Today</div></div>
    </div>
    ...
    <div style="background:rgba(255,193,7,.1);border:1px solid rgba(255,193,7,.3);border-radius:10px;padding:14px 20px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between">
      <span><strong><?= $pendingSongs ?> song<?= $pendingSongs > 1 ? 's' : '' ?></strong> waiting for approval</span>
      <a href="manage_songs.php?status=pending" class="btn btn-primary btn-sm">Review Now</a>
    </div>
    <?php endif; ?>

    <?php if ($pendingPayouts > 0): ?>
    <div style="background:rgba(33,150,243,.1);border:1px solid rgba(33,150,243,.3);border-radius:10px;padding:14px 20px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between">
      <span><strong><?= formatMoney($pendingPayouts) ?></strong> in pending payouts</span>
      <a href="payouts.php" class="btn btn-primary btn-sm">Manage Payouts</a>
    </div>
    <?php endif; ?>
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?= formatNumber((int) $s['download_count']) ?></td>
