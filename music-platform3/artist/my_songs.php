...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'delete') {
    ...
    if ($song->fetch()) { $songModel->delete($id); }
  ...
  <main class="panel-main">
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead><tr><th>Cover</th><th>Title</th><th>Genre</th><th>Status</th><th>Downloads</th><th>Earnings</th><th>Uploaded</th><th></th></tr></thead>
        <tbody>
          <?php foreach($songs as $s):
            $earned = round($s['download_count'] * EARNINGS_PER_DOWNLOAD, 2);
          ?>
          <tr>
            <td>
              <?php if($s['cover_art']):?>
              ...
              <?php else:?>
                <div style="width:40px;height:40px;background:var(--dark-3);border-radius:6px;display:flex;align-items:center;justify-content:center">🎵</div>
              <?php endif;?>
            </td>
            ...
            <td>
              ...
              <div style="font-size:11px;color:var(--text-dim);margin-top:3px">Under review</div>
              <?php endif;?>
            </td>
            ...
            <td>
              <div style="display:flex;gap:5px">
                ...
                <a href="../public/song.php?id=<?=$s['id']?>" target="_blank" class="btn btn-sm" style="border:1px solid var(--border)">👁 View</a>
                <?php endif;?>
                <form method="POST" style="display:inline">
                  <input type="hidden" name="song_id" value="<?=$s['id']?>">
                  <input type="hidden" name="action" value="delete">
                  <button class="btn btn-danger btn-sm" data-confirm="Delete this song permanently?">🗑</button>
                </form>
              </div>
            </td>
          </tr>
